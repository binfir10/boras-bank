...
import { footerData, footerSocialNetworks } from "@/lib/FooterData";
import Image from "next/image";

export default function Footer() {
  return (
    <footer className="max-w-5xl p-6 mx-auto mt-10 md:-mt-40">
        ...
        {footerData.map(({ id, title, links }) => (
          <div key={id}>
            ...
            {links.map(({ id, name, link }) => (
              <Link key={id} href={link} className="block mt-4 text-primaryDark hover:text-white hover:underline">
                <Reveal>
                  {name}
                </Reveal>
              </Link>
